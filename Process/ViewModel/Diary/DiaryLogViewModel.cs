...
namespace Process.ViewModel.Diary
{
    public class DiaryLogViewModel : ViewModelBase
    {
        ...
        /// </summary>
        public List<DiaryMonth> Months { get; set; } = new List<DiaryMonth>();
        ...
        {
            var months = new List<DiaryMonth>();
            ...
            for (int i = 1; i < 13; i++)
            {
                ...
                {
                    days.Add(FluentDayFactory.Day()
                        .SetDateFrom(TargetYear.YearNumber, i - 1 < 2 ? 1 : i -1, lastDayLastMonth.Day - ii)
                        .SetDayStatus(DayStatus.DayOfPreviousMonth)
                        .CreateDay());
                }
                ...
                for (int ii = 1; ii < thisMonthDayCount + 1; ii++)
                {
                    days.Add(FluentDayFactory.Day()
                        .IsEnabled(true)
                        .SetDayStatus(DayStatus.EmptyDay)
                        .SetDateFrom(TargetYear.YearNumber, i, ii)
                        .SetDiaryLog(new DiaryLog())
                        .CreateDay());
                }
                ...
                {
                    days.Add(FluentDayFactory.Day()
                        .SetDateFrom(TargetYear.YearNumber, i + 1 > 12 ? 12 : i + 1 , ii)
                        .SetDayStatus(DayStatus.DayOfNextMonth)
                        .CreateDay());
                }

                months.Add(FluentMonthFactory.Month()
                      .SetMonthNumber(i)
                      .SetMonthName(i)
                      .SetYear(TargetYear.YearNumber)
                      .SetDays(days)
                      .CreateMonth());
            }
